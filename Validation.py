...
#    cost_V = np.mean(abs(out_model[0] - VTAS[(TimeData >= tstart) & (TimeData <= tend)][0]))
#    cost_a = np.mean(abs(out_model[1] - VaneAOA[(TimeData >= tstart) & (TimeData <= tend)][0]))
#    cost_0 = np.mean(np.absolute(out_model[2] - Pitch[(TimeData >= tstart) & (TimeData <= tend)][0]))
#    print(cost_0)
#    
#    cost = cost_V + cost_a + cost_0
#    
#    return cost_0
...
#plot_data(0,TimeData[-1], Psi)


#Optimization of Cma, Cmde, muc
    
#opt = np.optimize.minimize(lambda x: compare_s(*x), [-0.60211])    